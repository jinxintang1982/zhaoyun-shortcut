...
public class Terminal {
    ...
    public static void calculation() {
        ...
        System.out.println(Stream.of(1, 2, 3, 4, 5).count()); //15
    }
    ...
    public static void reduce() {
        System.out.println(Stream.of("I", "love", "you", "too").reduce((s1, s2) -> s1.length() >= s2.length() ? s1 : s2));// love 因为love的长度最长
        System.out.println(Stream.of(1, 2, 3, 4, 5).reduce(Integer::sum));//15
    }

    //收集器
    public static void collect() {
        System.out.println(Stream.of("I", "love", "you", "too").collect(Collectors.toList()));
        Set<String> set = Stream.of("I", "love", "you", "too").collect(Collectors.toSet());
        Map<String,String> map = User.findUsers().stream().collect(Collectors.toMap(User::getUserId,User::getUserName));
    }
}
